...
// Phase 5a migration: was `pub` in heelonvault-core library crate (dead_code exempt for pub items).
// BackupDialog::show() will be wired from MainWindow profile view in Phase 5b.
...
use heelonvault_core::services::audit_service::{AuditAction, AuditService};
...
impl BackupDialog {
        ...
        TBackup: BackupService + Send + Sync + 'static,
    {
        ...
        let chooser = gtk4::FileChooserNative::builder()
            .title(heelonvault_core::tr!("backup-dialog-title").as_str())
            .transient_for(&deps.parent_window)
            .accept_label(heelonvault_core::tr!("backup-dialog-accept").as_str())
            .cancel_label(heelonvault_core::tr!("common-cancel").as_str())
        ...
        chooser.connect_response(move |dialog, response| {
            ...
            let Some(file) = selected else {
                (deps.on_feedback)(
                    heelonvault_core::tr!("backup-dialog-accept").as_str(),
                    heelonvault_core::tr!("backup-dialog-invalid-destination").as_str(),
                );
                return;
            };
            ...
            let Some(_backup_path) = file.path() else {
                (deps.on_feedback)(
                    heelonvault_core::tr!("backup-dialog-accept").as_str(),
                    heelonvault_core::tr!("backup-dialog-invalid-path").as_str(),
                );
                return;
            };
            ...
            match deps.backup_service.generate_recovery_key() {
                Ok(_key_bundle) => {
                    ...
                    (deps.on_feedback)(
                        heelonvault_core::tr!("backup-dialog-success-title").as_str(),
                        heelonvault_core::tr!("backup-dialog-success-message").as_str(),
                    );
                }
                Err(e) => {
                    // Log failed backup
                    ...
                    (deps.on_feedback)(
                        heelonvault_core::tr!("backup-dialog-error-title").as_str(),
                        heelonvault_core::tr!("backup-dialog-error-message").as_str(),
                    );
                }
            }
        });
