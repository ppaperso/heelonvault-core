...
pub(super) fn update_greeting(title_label: &gtk4::Label, username: &str) {
    if username.is_empty() {
        title_label.set_text(heelonvault_core::tr!("login-greeting-empty").as_str());
        return;
    }
...
pub(super) fn update_strength_feedback(password: &str, strength_label: &gtk4::Label) {
    ...
    let (label, css_class) = if score >= 4 {
        (
            heelonvault_core::tr!("login-password-strength-very-strong"),
            "success",
        )
    } else if score >= 3 {
        (
            heelonvault_core::tr!("login-password-strength-strong"),
            "success",
        )
    } else if score >= 2 {
        (
            heelonvault_core::tr!("login-password-strength-medium"),
            "warning",
        )
    } else {
        (
            heelonvault_core::tr!("login-password-strength-weak"),
            "error",
        )
    };
