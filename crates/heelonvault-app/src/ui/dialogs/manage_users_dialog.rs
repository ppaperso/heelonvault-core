...
impl ManageUsersDialog {
        ...
        TVault: VaultService + Send + Sync + 'static,
    {
        let window = gtk4::Window::builder()
            ...
            .title(heelonvault_core::tr!("manage-users-window-title").as_str())
            .modal(true)
            .default_width(760)
            .default_height(560)
            .build();
        ...
        let header = gtk4::Box::builder()
            ...
            .spacing(8)
            .build();
        let title = gtk4::Label::new(Some(heelonvault_core::tr!("manage-users-title").as_str()));
        title.set_halign(gtk4::Align::Start);
        title.set_hexpand(true);
        title.add_css_class("title-3");

        let create_button =
            gtk4::Button::with_label(heelonvault_core::tr!("manage-users-create-button").as_str());
        ...
        let col_user = gtk4::Label::new(Some(
            heelonvault_core::tr!("manage-users-col-username").as_str(),
        ));
        col_user.set_hexpand(true);
        col_user.set_halign(gtk4::Align::Start);
        let col_role = gtk4::Label::new(Some(
            heelonvault_core::tr!("manage-users-col-role").as_str(),
        ));
        col_role.set_width_chars(10);
        col_role.set_halign(gtk4::Align::Start);
        let col_date = gtk4::Label::new(Some(
            heelonvault_core::tr!("manage-users-col-created-at").as_str(),
        ));
                        ...
                        Ok(Ok(users)) => {
                            for user in users {
                                ...
                                let role_text = match user.role {
                                    UserRole::Admin => {
                                        heelonvault_core::tr!("manage-users-role-admin").to_string()
                                    }
                                    UserRole::User => {
                                        heelonvault_core::tr!("manage-users-role-user").to_string()
                                    }
                                };
                                ...
                                let reset_btn = gtk4::Button::with_label(
                                    heelonvault_core::tr!("manage-users-reset-password").as_str(),
                                );
                                reset_btn.add_css_class("flat");
                                let toggle_label = if matches!(user.role, UserRole::Admin) {
                                    heelonvault_core::tr!("manage-users-toggle-to-user").to_string()
                                } else {
                                    heelonvault_core::tr!("manage-users-toggle-to-admin")
                                        .to_string()
                                };
                                let toggle_btn = gtk4::Button::with_label(toggle_label.as_str());
                                toggle_btn.add_css_class("flat");
                                let delete_btn = gtk4::Button::with_label(
                                    heelonvault_core::tr!("manage-users-delete").as_str(),
                                );
                                ...
                                delete_btn.connect_clicked(move |_| {
                                    let confirm = adw::MessageDialog::new(
                                        ...
                                        Some(
                                            heelonvault_core::tr!(
                                                "manage-users-delete-confirm-title"
                                            )
                                            .as_str(),
                                        ),
                                        Some(
                                            heelonvault_core::tr!(
                                                "manage-users-delete-confirm-body"
                                            )
                                            .as_str(),
                                        ),
                                    );
                                    confirm.add_response(
                                        "cancel",
                                        heelonvault_core::tr!("common-cancel").as_str(),
                                    );
                                    confirm.add_response(
                                        "delete",
                                        heelonvault_core::tr!("manage-users-delete").as_str(),
                                ...
                                reset_btn.connect_clicked(move |_| {
                                    let reset_window = gtk4::Window::builder()
                                        .title(
                                            heelonvault_core::tr!(
                                                "manage-users-reset-window-title"
                                            )
                                            .as_str(),
                                        )
                                        .modal(true)
                                    ...
                                    password_entry.set_placeholder_text(Some(
                                        heelonvault_core::tr!(
                                            "manage-users-new-password-placeholder"
                                        )
                                        .as_str(),
                                    ));
                                    ...
                                    let apply = gtk4::Button::with_label(
                                        heelonvault_core::tr!("manage-users-apply").as_str(),
                                    );
                                    ...
                                    apply.connect_clicked(move |_| {
                                        ...
                                        if password.trim().is_empty() {
                                            Self::show_error(
                                                ...
                                                AppError::Validation(
                                                    heelonvault_core::tr!(
                                                        "manage-users-error-empty-password"
                                                    )
                                                    .to_string(),
                                                ),
                                            );
...

        {
            ...
            create_button.connect_clicked(move |_| {
                let create_window = gtk4::Window::builder()
                    .title(heelonvault_core::tr!("manage-users-create-window-title").as_str())
                    .modal(true)
                ...
                username.set_placeholder_text(Some(
                    heelonvault_core::tr!("manage-users-username-placeholder").as_str(),
                ));
                let password = gtk4::PasswordEntry::new();
                password.set_placeholder_text(Some(
                    heelonvault_core::tr!("manage-users-password-placeholder").as_str(),
                ));
                let role = gtk4::DropDown::from_strings(&[
                    heelonvault_core::tr!("manage-users-role-user").as_str(),
                    heelonvault_core::tr!("manage-users-role-admin").as_str(),
                ]);
                role.set_selected(0);
                ...
                let create =
                    gtk4::Button::with_label(heelonvault_core::tr!("manage-users-create").as_str());
    ...
    fn show_error(window: Option<&gtk4::Window>, err: AppError) {
        let title = match err {
            ...
            AppError::Authorization(_) => {
                heelonvault_core::tr!("manage-users-error-authorization").to_string()
            }
            _ => heelonvault_core::tr!("manage-users-error-generic").to_string(),
        };
