        ...
        chooser.connect_response(move |dialog, response| {
            ...
            let recovery = match deps.backup_service.generate_recovery_key() {
                ...
                Err(_) => {
                    (deps.on_feedback)(
                        heelonvault_core::tr!("profile-export-accept").as_str(),
                        heelonvault_core::tr!("profile-export-recovery-key-failed").as_str(),
                    );
                    return;
                }
            };
            ...
            confirm_dialog.add_response("cancel", heelonvault_core::tr!("common-cancel").as_str());
                    ...
                    match result {
                        Ok(()) => {
                            on_feedback_for_result(
                                heelonvault_core::tr!("profile-export-success-title").as_str(),
                                heelonvault_core::tr!("profile-export-success-body").as_str(),
                            );
                        }
