...
impl TrashDialog {
        ...
        TVault: VaultService + Send + Sync + 'static,
    {
        ...
        let window = gtk4::Window::builder()
            ...
            .title(heelonvault_core::tr!("trash-window-title").as_str())
            .modal(true)
            .default_width(760)
            .default_height(560)
            .build();
        ...
        let title = gtk4::Label::new(Some(heelonvault_core::tr!("trash-title").as_str()));
        title.add_css_class("title-3");
        title.set_halign(Align::Start);
        title.set_hexpand(true);

        let empty_button =
            gtk4::Button::with_label(heelonvault_core::tr!("trash-empty-button").as_str());
        ...
        let empty_title =
            gtk4::Label::new(Some(heelonvault_core::tr!("trash-empty-title").as_str()));
        empty_title.add_css_class("title-4");
        let empty_copy = gtk4::Label::new(Some(
            heelonvault_core::tr!("trash-empty-description").as_str(),
        ));
        ...
        stack.add_titled(
            &list_scroll,
            Some("list"),
            heelonvault_core::tr!("trash-list-title").as_str(),
        );
        stack.add_titled(
            &empty_state,
            Some("empty"),
            heelonvault_core::tr!("trash-list-empty").as_str(),
        );
        ...
        empty_button.connect_clicked(move |_| {
            ...
            Self::confirm_destructive(
                &window_for_empty,
                heelonvault_core::tr!("trash-confirm-empty").as_str(),
                move || {
                    ...
                    std::thread::spawn(move || {
                        let result = runtime_for_task.block_on(async move {
                            ...
                            Ok::<(), heelonvault_core::errors::AppError>(())
                        });
    ...
    async fn resolve_current_vault<TVault>(
        ...
        admin_user_id: Uuid,
        admin_master_key: Vec<u8>,
    ) -> Result<Uuid, heelonvault_core::errors::AppError>
    where
        TVault: VaultService + Send + Sync + 'static,
    {
        let vaults = vault_service.list_user_vaults(admin_user_id).await?;
        let current = vaults.into_iter().next().ok_or_else(|| {
    ...
    fn refresh_trash_list<TSecret, TVault>(
        ...
        TVault: VaultService + Send + Sync + 'static,
    {
        empty_title.set_text(heelonvault_core::tr!("trash-loading-title").as_str());
        empty_copy.set_text(heelonvault_core::tr!("trash-loading-copy").as_str());
        stack.set_visible_child_name("empty");
        ...
        let (sender, receiver) = tokio::sync::oneshot::channel();
        std::thread::spawn(move || {
            let result: Result<Vec<TrashRowView>, heelonvault_core::errors::AppError> =
                runtime_for_loader.block_on(async move {
                    ...
                    for item in items {
                        ...
                        rows.push(TrashRowView {
                            ...
                            title: item.title.unwrap_or_else(|| {
                                heelonvault_core::tr!("trash-secret-fallback-title")
                            }),
        ...
        glib::MainContext::default().spawn_local(async move {
            match receiver.await {
                Ok(Ok(items)) => {
                    ...
                    if items.is_empty() {
                        empty_title.set_text(heelonvault_core::tr!("trash-empty-title").as_str());
                        empty_copy
                            .set_text(heelonvault_core::tr!("trash-empty-description").as_str());
                        stack.set_visible_child_name("empty");
                        return;
                    }
                    ...
                    for item in items {
                            ...
                            .map(|raw| {
                                if raw.len() >= 16 {
                                    ...
                                    heelonvault_core::i18n::tr_args(
                                        "trash-deleted-at",
                                        &[
                                            ("date", heelonvault_core::i18n::I18nArg::Str(date)),
                                            ("time", heelonvault_core::i18n::I18nArg::Str(time)),
                                        ],
                                    )
                                } else {
                                    heelonvault_core::i18n::tr_args(
                                        "trash-deleted-at-raw",
                                        &[("raw", heelonvault_core::i18n::I18nArg::Str(raw))],
                                    )
                                }
                            })
                            .unwrap_or_default();
                        ...
                        let restore_btn = gtk4::Button::with_label(
                            heelonvault_core::tr!("trash-restore-button").as_str(),
                        );
                        ...
                        let delete_btn = gtk4::Button::with_label(
                            heelonvault_core::tr!("trash-delete-button").as_str(),
                        );
                        ...
                        delete_btn.connect_clicked(move |_| {
                            ...
                            Self::confirm_destructive(
                                &window_for_delete,
                                heelonvault_core::tr!("trash-confirm-delete").as_str(),
                                move || {
                                    let (sender, receiver) = tokio::sync::oneshot::channel();
                                    let runtime_for_task = runtime_for_confirm.clone();
                                    let secret_for_task = Arc::clone(&secret_for_confirm);
                ...
                Ok(Err(_)) | Err(_) => {
                    empty_title.set_text(heelonvault_core::tr!("trash-unavailable-title").as_str());
                    empty_copy.set_text(heelonvault_core::tr!("trash-unavailable-copy").as_str());
                    stack.set_visible_child_name("empty");
                }
            }
        });
    ...
    fn confirm_destructive(parent: &gtk4::Window, message: &str, on_confirm: impl Fn() + 'static) {
        let dialog = gtk4::MessageDialog::builder()
            .transient_for(parent)
            .modal(true)
            .text(heelonvault_core::tr!("trash-dialog-title").as_str())
            .secondary_text(message)
            .build();
        dialog.add_button(
            heelonvault_core::tr!("trash-dialog-cancel").as_str(),
            gtk4::ResponseType::Cancel,
        );
        dialog.add_button(
            heelonvault_core::tr!("trash-dialog-confirm").as_str(),
