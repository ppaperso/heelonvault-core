...
pub(super) fn build_sidebar_panel() -> SidebarWidgets {
    ...
    let my_vaults_title =
        gtk4::Label::new(Some(heelonvault_core::tr!("main-my-vaults-title").as_str()));
    ...
    create_vault_button.set_tooltip_text(Some(
    ...
    let shared_vaults_title = gtk4::Label::new(Some(
        heelonvault_core::tr!("main-shared-with-me-title").as_str(),
    ));
    ...
    let audit_title = gtk4::Label::new(Some(heelonvault_core::tr!("main-audit-title").as_str()));
    ...
    let (audit_all_row, audit_all_label, audit_all_badge) = build_audit_sidebar_row(
        heelonvault_core::tr!("main-audit-all").as_str(),
        "view-grid-symbolic",
    );
    let (audit_weak_row, audit_weak_label, audit_weak_badge) = build_audit_sidebar_row(
        heelonvault_core::tr!("main-audit-weak").as_str(),
    ...
    let sidebar_title = gtk4::Label::new(Some(
        heelonvault_core::tr!("main-categories-title").as_str(),
    ));
    ...
    let rows = [
        (
            heelonvault_core::tr!("main-category-all"),
            "view-grid-symbolic",
        ),
        (
            heelonvault_core::tr!("main-category-passwords"),
        ...
        (
            heelonvault_core::tr!("main-category-documents"),
    ...
    let account_title =
        gtk4::Label::new(Some(heelonvault_core::tr!("main-account-title").as_str()));
    ...
    let profile_security_label = gtk4::Label::new(Some(
    ...
    let teams_label = gtk4::Label::new(Some(heelonvault_core::tr!("main-teams-nav").as_str()));
    ...
    let administration_label =
        gtk4::Label::new(Some(heelonvault_core::tr!("main-user-nav").as_str()));
...
pub(super) fn build_vault_sidebar_row(
    ...
    drag_handle.set_tooltip_text(Some(
        heelonvault_core::tr!("main-vault-drag-tooltip").as_str(),
    ));
    ...
    if shared_role.is_some() || is_shared_with_others {
        ...
        shared_icon.set_tooltip_text(Some(
            heelonvault_core::tr!("main-vault-shared-tooltip").as_str(),
        ));
    ...
    if can_delete {
        ...
        delete_button.set_tooltip_text(Some(
            heelonvault_core::tr!("main-delete-vault-tooltip").as_str(),
        ));
